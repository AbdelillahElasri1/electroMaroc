...
<center>
    <div class="AddProduct w-1/3">
        <h1 class="text-xl font-semibold text-sky-800 my-8">Update Category</h1>
        <form action="?p=Dashboard&a=updateCategory&id=<?=$_GET['id']?>" method="post" enctype="multipart/form-data" class="border p-6">
            <!-- <label class="block">
            ...
            <label class="block">
                ...
                <input type="text" value="<?=($test->select('categoryName',$_GET['id']))['categoryName']?>" name="categoryName" required placeholder="Category Name" class="block px-2 border w-full text-sm text-sky-700 h-10 rounded-sm my-4 px-2
            " />
            </label>
            <label class="block">
                ...
                <?=($test->select('categoryDescription',$_GET['id']))['categoryDescription']?>
            </textarea>
            </label>
